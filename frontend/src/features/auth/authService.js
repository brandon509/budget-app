...
const resetPasswordRequest = async (email) => {
    const response = await axios.put(`http://localhost:8000/resetPasswordRequest`, email)

    return response.data
}

const resetPassword = async ({ password, ident, timeStamp, hash }) => {
    const response = await axios.put(`http://localhost:8000/resetPassword/${ident}/${timeStamp}/${hash}`, {password: password})

    return response.data
}

const authService = { login, signup, logout, verifyEmail, resetPasswordRequest, resetPassword }
